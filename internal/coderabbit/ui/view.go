...
func RenderView(m *Model) string {
	...
	if m.review != nil {
		...
		viewState.CodeRabbitFound = m.review.CodeRabbitFound
...
func renderThoughts(thoughts []domain.ThoughtChunk, width, height, scrollOffset int, state ThoughtViewState) string {
	if len(thoughts) == 0 {
		...
		if !state.CodeRabbitFound {
			// CodeRabbit hasn't even started yet
			message = "◐ Waiting for CodeRabbit to start review...\n\nCodeRabbit check not found yet."
		} else if state.CodeRabbitFound && !state.CodeRabbitCompleted {
			// CodeRabbit is actively reviewing
			...
			// All clear: CodeRabbit reviewed, no comments, no CI failures, no pending CI
			if state.WatchMode {
			...
			// Other CI checks still running (CodeRabbit done, but other checks pending)
			message = fmt.Sprintf("✓ CodeRabbit review complete\n◐ %d CI check(s) still running\n\nWaiting for CI to complete...", state.CIPendingCount)
